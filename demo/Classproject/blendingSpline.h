#ifndef MyBlendingSplineSpline_H
#define MyBlendingSplineSpline_H
...
namespace GMlib {
  ...
  class MyBlendingSpline : public PCurve<T,3> {

    GM_SCENEOBJECT(MyBlendingSpline)

  public:

    MyBlendingSpline( PCurve <T ,3 >* model_curve, int n);
...
//*****************************************
  ...
  MyBlendingSpline<T>::MyBlendingSpline(PCurve <T ,3 >* model_curve, int n):PCurve<T,3>(20, 0, 0), _d(1), _k(2) {
  ...
  MyBlendingSpline<T>::MyBlendingSpline( const MyBlendingSpline<T>& copy ) : PCurve<T,3>(copy) {
  ...
  T MyBlendingSpline<T>::getBlend(T t)const{
  ...
  void MyBlendingSpline<T>::localSimulate( double dt ) {
      ...
      if(_modelCurve) {
          ...
          for(int i=0; i<_n; i++){
              ...
              v.push_back(UnitVector<T, 3>(i, _n-i , i+2)); //generates the path for translate

          }

          for(int i=0; i<_n; i++){

              _c[i]->rotateParent(dt, Vector<float,3>(0,0,1));

          }

          this->setEditDone(true);

          PCurve<T,3>::sample(40,0);
  ...
  template <typename T>
     ...
     void MyBlendingSpline<T>::localcurves(int n, bool closed){
  ...
  void MyBlendingSpline<T>::findControlPoints( const DVector<Vector<T,3>>& p, int m, int n )  {
  ...
  void MyBlendingSpline<T>::eval( T t, int d, bool /*l*/ ) const {
    ...
    std::cout<<i<< " now print "<<t<<std::endl;
  ...
  void MyBlendingSpline<T>::makeKnotsOpen(int n, T s, T e) {
  ...
  void MyBlendingSpline<T>::makeKnotsClosed(int n, T s, T e) {
  ...
  int MyBlendingSpline<T>::findI(T t) const{
  ...
  T MyBlendingSpline<T>::getW(int d, int i, T t) const{
  ...
  Vector<T,2> MyBlendingSpline<T>::getB(int i, T t) const{
